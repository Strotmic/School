...
foreach (string line in lines)
{
    ...
    GuestPerformance x = new GuestPerformance(data[0], data[1], double.Parse(data[2]), data[3], data[4]);
    guestPerformances.Add(x);
}
...
string[] lines = File.ReadAllLines("./Data/theatreproduction.csv");

foreach (string line in lines)
{
    string[] data = line.Split(',');
    // Name; Date; Price; Description; Director; Genre
